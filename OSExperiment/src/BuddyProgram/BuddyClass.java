...
public class BuddyClass {
	...
	public void InitData() {
		for (int i = 6; i <=14; i++) {
			for(int j=0;j<(int)(Math.pow(2, 14)/Math.pow(2, i));j++){
				Memory_Or_Not[i][j]=0;
				Valuable_Or_Not[i][j]=0;//不可用或非空闲
				//myframe.changedMemoryState(i, j, 2);
				if(i==14&&j==0){
					Memory_Or_Not[i][j]=1;//最大的内存块
					Valuable_Or_Not[i][j]=1;//空闲
	...
	public void Storage_Allocation(int pro_size){
		...
		for(i=location;i<=14;i++){
			...
			for(int j=0;j<Math.pow(2, 14)/Math.pow(2, i);j++){
				if(Memory_Or_Not[i][j]==1&&Valuable_Or_Not[i][j]==1){
					...
					myframe.setProcess_info_text("进程创建成功！ 进程名："+"ID"+xMemlocate+yMemlocate
							+" 大小："+pro_size+"KB"+"  占用内存："+(int)Math.pow(2, i)+"KB "+" 占用时间："+process_time+"ms");
		...
		if(i>14){
			...
			myframe.setProcess_info_text("无法创建新进程，找不到适合"+pro_size+"KB大小的内存块，请等待内存释放！");
		}
//		else{
//			for(i=6;i<=14;i++){
//				for(int j=0;j<Math.pow(2, 14)/Math.pow(2, i);j++){
